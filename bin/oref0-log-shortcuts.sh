...
grep -q edit-pref ~/.bash_profile || echo "alias edit-pref="'"cd ~/myopenaps && $EDITOR preferences.json"' >> ~/.bash_profile
grep -q cat-wifi ~/.bash_profile || echo "alias cat-wifi="'"cat /etc/wpa_supplicant/wpa_supplicant.conf"' >> ~/.bash_profile
grep -q edit-wifi ~/.bash_profile || echo "alias edit-wifi="'"$EDITOR /etc/wpa_supplicant/wpa_supplicant.conf"' >> ~/.bash_profile
grep -q cat-runagain ~/.bash_profile || echo "alias cat-runagain="'"cd ~/myopenaps && cat oref0-runagain.sh"' >> ~/.bash_profile
grep -q edit-runagain ~/.bash_profile || echo "alias edit-runagain="'"cd ~/myopenaps && $EDITOR oref0-runagain.sh"' >> ~/.bash_profile
