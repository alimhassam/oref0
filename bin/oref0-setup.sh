...
#fi
    ...
    if [[ "$ttyport" =~ "spi" ]]; then
        ...
        if [[ ${ww_ti_usb_reset,,} =~ "yes" ]]; then
                ...
                openaps alias add mmtune "! bash -c \"oref0_init_pump_comms.py --ww_ti_usb_reset=yes -v; find monitor/ -size +5c | grep -q mmtune && cp monitor/mmtune.json mmtune_old.json; echo {} > monitor/mmtune.json; echo -n \"mmtune: \" && openaps report invoke monitor/mmtune.json; [  $(wc -c <monitor/mmtune.json ) -gt 5 ] || cp mmtune_old.json monitor/mmtune.json; grep -v setFreq monitor/mmtune.json | grep -A2 $(json -a setFreq -f monitor/mmtune.json) | while read line; do echo -n \"$line \"; done\""
        fi
